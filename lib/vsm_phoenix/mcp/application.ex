defmodule VsmPhoenix.MCP.Application do
  ...
  defp build_children(config, _opts) do
    ...
    children = if Keyword.get(config, :server_enabled, true) do
      server_children = case Keyword.get(config, :server_transport, :stdio) do
        :stdio ->
          [
            # STDIO transport
    ...
    children = if Keyword.get(config, :client_enabled, true) do
      client_config = case Keyword.get(config, :client_transport, :stdio) do
        :stdio ->
          [transport: :stdio]
    ...
    children = if Keyword.get(config, :registry_enabled, true) do
      [
  ...
  def start_server(transport_type, opts \\ []) do
    spec = case transport_type do
      :stdio ->
        %{
          id: :dynamic_stdio_server,
          start: {VsmPhoenix.MCP.Servers.VsmHermesServer, :start_link, [
            Keyword.merge([
              transport: [
                layer: VsmPhoenix.MCP.Transports.StdioTransport,
  ...
  def start_client(server_url, opts \\ []) do
    spec = %{
      ...
      start: {VsmPhoenix.MCP.Clients.VsmHermesClient, :start_link, [
        Keyword.merge([
