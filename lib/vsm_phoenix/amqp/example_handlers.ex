defmodule VsmPhoenix.AMQP.ExampleHandlers do
    ...
    CommandRPC.register_handler(:system1, fn command, _meta ->
      case command["action"] do
        ...
        "get_sensor_data" ->
          ...
          %{
            sensors: Enum.map(command["sensor_ids"] || [], fn sensor_id ->
              %{
                id: sensor_id,
  ...
  def setup_system3_handler do
    handler = CommandRPC.with_validation(
      fn command, _meta ->
        case command["action"] do
          "allocate_resources" ->
          ...
          "emergency_shutdown" ->
            ...
            {:ok, shutdown_result} = CommandRPC.call(:system1, %{
      ...
      case command["action"] do
        "set_policy" ->
          ...
          if command["policy_type"] == "resource_conservation" do
            {:ok, s3_result} = CommandRPC.call(:system3, %{
        ...
        "get_system_health" ->
          ...
          {:ok, s3_resources} = CommandRPC.call(:system3, %{
