defmodule VsmPhoenix.MCP.VarietyAcquisition do
  ...
  def handle_cast({:register_gap, gap_info}, state) do
    ...
    pending =
      Map.put(state.pending_acquisitions, gap_id, %{
        gap: gap_info,
    ...
    new_state =
      if gap_info[:urgency] == :critical do
  ...
  def handle_info(:variety_scan, state) do
    ...
    # Perform scan
    new_state =
      case perform_variety_scan(:routine, state) do
        {:ok, gaps} when length(gaps) > 0 ->
  ...
  defp execute_acquisition_pipeline(gap, state) do
    ...
    with {:discover, {:ok, candidates}} <- {:discover, discover_capabilities(gap)},
         ...
         {:integrate, {:ok, integration}} <-
           {:integrate, integrate_capability(connection, gap, state)},
      ...
      {phase, {:error, reason}} ->
        Logger.error(
          "❌ Acquisition pipeline #{pipeline_id} failed at #{phase}: #{inspect(reason)}"
        )
  ...
  defp analyze_variety_gaps(variety_data, state) do
    ...
    anomaly_gaps =
      Enum.flat_map(anomalies, fn anomaly ->
  ...
  defp calculate_fitness_score(candidate, gap, state) do
    ...
    official_bonus =
      if String.starts_with?(candidate["name"], "@modelcontextprotocol/"), do: 20, else: 0
    ...
    base_score =
      tool_relevance * 0.4 + description_match * 0.3 +
  ...
  defp calculate_tool_relevance(tools, requirements)
       when is_list(tools) and is_list(requirements) do
    if length(requirements) == 0 do
      # Base score if no specific requirements
      50
    else
      matching_tools =
        Enum.count(requirements, fn req ->
  ...
  defp get_historical_performance(server_name, state) do
    ...
    history =
      Enum.filter(state.acquisition_history, fn entry ->
    ...
    if length(history) > 0 do
      ...
      # Neutral score for unknown servers
      50
    end
  end
  ...
  defp record_acquisition_success(result, state) do
    ...
    new_capabilities =
      Map.merge(state.capabilities, result.integration.state_updates.capabilities)
  ...
  defp update_domain_strategy(strategies, result_or_failure, outcome) do
    ...
    updated_strategy =
      case outcome do
        ...
        :failure ->
          %{current_strategy | attempts: current_strategy.attempts + 1}
      end
  ...
  defp calculate_system_variety(state) do
    ...
    tool_count =
      Enum.reduce(state.capabilities, 0, fn {_, cap}, acc ->
        acc + length(cap.tools)
      end)

    # Add base system variety
    # VSM's inherent variety
    base_variety = 10
