defmodule VsmPhoenix.Demos.MultiAgentSwarm do
  ...
  defp create_orchestrator_agent do
    ...
    config = %{
      ...
      role: "orchestrator",
      level: 0,
      # Orchestrator doesn't need MCP, just coordinates
      mcp_servers: [],
      ...
      "data" => %{
        "specialists" => [
          ...
          %{
            "name" => "DataAnalyst",
            "mcp_server" =>
              "npx -y @modelcontextprotocol/server-sqlite #{File.cwd!()}/test_vsm.db",
  ...
  defp demonstrate_amqp_communication do
    ...
    Task.start(fn ->
      ...
      ["filemanager", "timekeeper", "dataanalyst"]
      |> Enum.each(fn specialist ->
        queue_name = "vsm.swarm.#{specialist}"
        {:ok, _} = AMQP.Queue.declare(channel, queue_name, durable: true)

        :ok =
          AMQP.Queue.bind(channel, queue_name, "vsm.swarm",
            routing_key: "vsm.swarm.#{specialist}"
          )
